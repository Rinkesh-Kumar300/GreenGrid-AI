...
class RAGGuidanceItem(BaseModel):
    ...
    score: float = Field(
        description="Cosine distance (lower = more relevant)."
    )
...
class AnalyzeResponse(BaseModel):
    ...
    actual_consumption: float = Field(
        description="Measured kWh from the request."
    )

    predicted_consumption: float = Field(
        description="Model-predicted kWh."
    )

    difference_kwh: float = Field(
        description="Difference between actual and predicted consumption."
    )

    deviation_percent: float = Field(
        description="Percentage deviation from predicted."
    )

    status: str = Field(
        description="Normal | Elevated | Abnormal"
    )
    ...
    rag_guidance: List[RAGGuidanceItem] = Field(
        default=[],
        description="Retrieved knowledge-base sections."
    )
    ...
    estimated_savings: str = Field(
        description="Estimated savings range, e.g. '8-12%'."
    )
